...
@section('container')
    <div class="container mx-auto px-4 pt-20">
        <div class="bg-white rounded-lg shadow-md max-w-3xl mx-auto">
            <div class="p-8">
                <h4 class="text-lg font-semibold mb-8">Form Surat Keterangan Tidak Mampu</h4>
                <form action="{{ isset($sktm) ? route('sktm.update', $sktm->sktm_id) : route('sktm.store') }}" method="POST">
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="alamat" name="alamat" rows="4"
                            class="w-full border border-gray-300 rounded-md px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-blue-500 @error('alamat') border-red-500 @enderror">{{ old('alamat', $sktm->alamat ?? '') }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="keterangan" name="keterangan" rows="4"
                            class="w-full border border-gray-300 rounded-md px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-blue-500 @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $sktm->keterangan ?? '') }}</textarea>
                        @error('keterangan')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
